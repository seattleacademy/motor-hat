...
module.exports = function exports(options) {
  ...
  const setPWMFreq = function setPWMFreq(freq, cb = isRequired()) {
    if (typeof freq !== 'number') errHdlr('freq should be a number.');
  ...
  const setPWM = function setPWM(channel, on, off, cb = isRequired()) {
    if (typeof (channel) !== 'number' || channel < 0 || channel > 15) errHdlr('Channel should be a number between 0 and 15.');
    if (typeof (on) !== 'number' || typeof (off) !== 'number' || on < 0 || on > 4096 || off < 0 || off > 4096) errHdlr('on and off should be numbers between 0 and 4096.');
  ...
  const setPin = function setPin(channel, state, cb = isRequired()) {
    if (typeof (channel) !== 'number' || channel < 0 || channel > 15) errHdlr('Channel should be a number between 0 and 15.');
    if (typeof (state) !== 'number' || (state !== 0 && state !== 1)) errHdlr('State should be 0 or 1.');
