 ...
 * @example Basic Usage:
 *
 * const motorHat = require('motor-hat')({addr: 0x6F});
 *
 * @param {Object} [opts]
 * @param {Integer} [opts.adress] i2c adress of the PWM chip on the MotorHat.
 ...
 * @param {String[]} [opts.steppers] Definition of the stepper motors connected to the HAT.
 *  At most 2 steppers, each sould be one of following: 'M1', 'M2', 'M3', 'M4' depernding on
 *  port the stepper is connected to.
 * @param {String[]} [opts.dcs] Definition of the DC motors connected to the HAT.
 *  At most 4 DCs, each sould be one of following: 'M1', 'M2', 'M3', 'M4' depernding on
...
module.exports = function MotorHat(opts) {
  /**
   * Definition of boards pins to translate configuration of steppers and DC motors.
   * @ignore
   */
  const pins = {
    M1: {
  ...
  const steppers = [];
  /**
   * Array of DC controllers
   * @type {module:MotorHat/DC~dC[]}
   */
  const dcs = [];
  /**
   * Array of Servo controllers
   ...
   * @returns {module:MotorHat~motorHat} Returns initialized motorHat object, either directly
   * or in second parameter to callback if callback provided.
   */
